/* eslint-disable */
/**
 * Combined Prisma migration SQL (all tables), embedded so the one-time runtime
 * bootstrap route can create the schema without the Prisma CLI. Generated from
 * packages/db/prisma/migrations/*. The executor runs statements individually and
 * ignores "already exists" errors, so it is safe to re-run.
 */
export const BOOTSTRAP_SQL = String.raw`
-- CreateEnum
