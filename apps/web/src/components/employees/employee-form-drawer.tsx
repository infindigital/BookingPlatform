...
export function EmployeeFormDrawer({
  ...
  seed: EmployeeEditSeed | null;
  onSaved: (employeeId: string) => void;
}) {
  const isEdit = !!seed;
  const [state, action, pending] = useActionState<EmployeeActionState, FormData>(
    isEdit ? updateEmployeeAction : createEmployeeAction,
    { ok: false },
  );

  useEffect(() => {
    if (state.ok && state.employeeId) onSaved(state.employeeId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.ok, state.employeeId]);
        ...
        {open ? (
          <form action={action} className="flex h-full flex-col">
            <header className="flex items-center justify-between border-b border-border p-5">
              <SheetTitle className="text-base font-semibold">{isEdit ? 'Edit team member' : 'New team member'}</SheetTitle>
              <SheetClose asChild>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              {isEdit ? <input type="hidden" name="employeeId" value={seed!.id} /> : null}
              ...
              <label className="block space-y-1">
                ...
                <input name="title" defaultValue={seed?.title ?? ''} className={CONTROL} placeholder="e.g. Senior Specialist" />
              </label>
              <label className="block space-y-1">
                <span className="text-xs font-medium text-muted-foreground">Email</span>
                <input name="email" type="email" defaultValue={seed?.email ?? ''} className={CONTROL} autoComplete="email" />
              </label>
              <label className="block space-y-1">
              ...
              {isEdit ? (
                <label className="flex items-center gap-2.5 rounded-lg border border-border bg-muted/30 p-3">
                  ...
                  <span>
                    ...
                    <span className="block text-xs text-muted-foreground">Inactive members can’t be booked and are hidden from availability.</span>
                  </span>
                </label>
              ) : null}
